...
- Queue contracts are implemented as RabbitMQ-routed messages.
...
- Provider/message verification must be mock-based (embedded app + in-memory mocks) and must not depend on PostgreSQL, Redis, RabbitMQ, or live service endpoints.
- Message contract coverage includes ingest -> worker `media.process`, library -> worker `media.derivatives.generate`, library -> worker `media.cleanup`, and worker-managed orphan sweep `media.orphan.sweep`.
...
- `POST /worker/orphan-sweep/run` (admin-only)
- `GET /worker/docs`
...
`media.orphan.sweep` compatibility requirements:
- Required keys: `scope`, `dryRun`, `requestedAt`, `requestId`, `graceMs`, `batchSize`.
- `scope` enum: `originals | derived`.
- Worker must only evaluate files rooted under configured originals/derived directories.
- Worker must treat files newer than `graceMs` as protected (`skippedRecent`) and not delete them.
- `dryRun=true` must never delete files.
- For `scope=derived`, worker cleanup semantics include:
  - delete true orphans (derived files with no matching `media.id`), and
  - delete non-canonical duplicate derived artifacts for existing media IDs while preserving canonical variants.

### `POST /worker/orphan-sweep/run` response shape
```json
{
  "status": "queued",
  "requestId": "req_abc123",

...
export default function AppSidebar({ activeLabel = "Timeline", isAdmin = false }) {
  ...
  return (
    <>
      <aside
        className="shrink-0 z-50 w-[45px] sm:w-[72px] bg-white dark:bg-background-dark border-r border-gray-200 dark:border-border-dark lg:w-60 transition-[width] duration-300 group flex flex-col"
      >
        <nav className="flex flex-col gap-2 p-1 sm:p-3 mt-4">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={`flex items-center justify-center lg:justify-start gap-0 lg:gap-4 px-1 sm:px-3 py-3 rounded-lg transition-all group/item relative overflow-hidden ${isActive
              ...
              >
                ...
                <span className="whitespace-nowrap hidden lg:block transition-opacity duration-300">
                  {item.label}
                </span>
                {isActive && (

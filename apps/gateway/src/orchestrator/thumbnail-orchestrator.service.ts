...
@Injectable()
export class ThumbnailOrchestratorService implements OnModuleDestroy {
  ...
  onModuleDestroy(): void {
    ...
    if (this.dlq.length > 0) {
      this.logger.error(
        `${this.dlq.length} thumbnail job(s) still in DLQ on shutdown — these will be lost`,
      )
    }
  }
  ...
  private async tryEnqueue(job: ThumbnailJob): Promise<boolean> {
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt < MAX_ATTEMPTS - 1) {
          ...
          this.logger.error(
            `Failed to enqueue thumbnail after ${MAX_ATTEMPTS} attempts (asset=${job.assetId}, size=${job.size}): ${msg}`,
          )
        }
      }
    }
  ...
  private addToDlq(job: ThumbnailJob): void {
    this.dlq.push({
      job,
      attempts: 0,
    ...
    this.logger.error(
      `Thumbnail job moved to DLQ (asset=${job.assetId}, size=${job.size}) — will retry every ${DLQ_RETRY_INTERVAL_MS / 1000}s`,
    )
  }
  ...
  private async drainDlq(): Promise<void> {
    ...
    this.logger.log(`DLQ drain: retrying ${due.length} thumbnail job(s)`)
    ...
    for (const entry of due) {
      ...
      if (ok) {
        ...
        this.logger.log(
          `DLQ job recovered (asset=${entry.job.assetId}, size=${entry.job.size}) after ${entry.attempts} DLQ attempt(s)`,
        )
      } else if (entry.attempts >= DLQ_MAX_RETRIES) {
        const idx = this.dlq.indexOf(entry)
        if (idx >= 0) this.dlq.splice(idx, 1)
        this.logger.error(
          `DLQ job permanently failed after ${entry.attempts} retries (asset=${entry.job.assetId}, size=${entry.job.size}) — manual intervention required`,
        )
      } else {

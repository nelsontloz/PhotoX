...
describe('HttpExceptionFilter (gateway)', () => {
  ...
  it('F-G1: HttpException with string body — status and json called with correct args', () => {
    filter.catch(new HttpException('Forbidden', HttpStatus.FORBIDDEN), host)
    expect(status).toHaveBeenCalledWith(HttpStatus.FORBIDDEN)
    expect(json).toHaveBeenCalledWith('Forbidden')
  })

  it('F-G2: HttpException with object body is preserved verbatim', () => {
  ...
  it('F-G3: UnauthorizedException default body is preserved as-is', () => {
  ...
  it('F-G4: non-HttpException — 500 with generic body', () => {
  ...
  it('F-G5: status().json() chain is invoked exactly once per request', () => {

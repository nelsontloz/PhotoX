...
describe('Pact verification — media-service albums', () => {
  it('validates album expectations of Gateway', async () => {
    await new Verifier({
      provider: 'media-service',
      providerBaseUrl: url,
      pactUrls: [path.join(PACT_DIR, 'gateway-albums-media-service.json')],
      logLevel: 'error',

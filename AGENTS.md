...
- Refactored photo selection UI: removed the global "Select" button and implemented automatic selection mode activation by clicking on the selection circle of any thumbnail. Updated `TimelineThumbnail` to handle circle clicks with event propagation prevention and implemented a **long-press gesture** (500ms) with haptic feedback to trigger selection mode, facilitating mobile-first interaction.
- Implement Infinite Scroll on Timeline: Replaced the manual "Load more" button with an `IntersectionObserver`-based automatic loading system. Added a `rootMargin` of 200px to trigger fetches early for a seamless UX and maintained the loading spinner within the sentinel element for visual feedback.
...
- Refactored Media Selection Action Bar: Moved `TimelineSelectionActionBar` to shared components as `SelectionActionBar`. Implemented mobile-responsive button labels (icons only on mobile). Refined layout with "Delete" on the left and a blue "Add to Album" primary button on the right.
- Created Global Confirmation Modal: Implemented a premium, reusable `ConfirmationModal` component in `apps/web/app/components/` to replace native browser dialogs with a high-fidelity experience. Updated the Timeline page to use it for bulk trashing.
- Redesigned the Global Upload Progress notification UI: implemented a high-fidelity, dark-mode component matching the requested design. Added a pulsating active indicator, redesigned the upload queue with progress bars and status labels, and included a detailed footer with total transfer progress. Updated the Material Symbols subset with the `remove` icon for the minimize action.
- Refined Timeline UI: Updated `sectionLabel` utility to remove redundant date subtitles for older entries, implemented responsive typography (`text-xl` on mobile) in `TimelineSectionList`, and removed the "Select all" option for a cleaner interface.
